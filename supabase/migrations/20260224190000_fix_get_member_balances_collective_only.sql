...
AS $function$
  SELECT
    es.user_id,
    -- Total Owed: Soma apenas splits de despesas COLETIVAS
    coalesce(sum(es.amount), 0) as total_owed,
    
    -- Total Paid: Soma apenas splits PAGOS de despesas COLETIVAS
  ...
  WHERE e.group_id = _group_id
    AND e.expense_type = 'collective' -- Filtro CRÍTICO: ignora 'individual'
